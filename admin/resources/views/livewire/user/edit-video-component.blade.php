    ...
    <div class="container-scroller">
      ...
      <nav class="sidebar sidebar-offcanvas" id="sidebar">
        ...
        <ul class="nav">
          ...
          <li class="nav-item menu-items">
            <a class="nav-link" data-bs-toggle="collapse" href="#video" aria-expanded="false" aria-controls="ads">
              <span class="menu-icon">
                <i class="mdi mdi-video"></i>
              </span>
              <span class="menu-title">Vending Video</span>
              <i class="menu-arrow"></i>
            </a>
            <div class="collapse" id="video">
              <ul class="nav flex-column sub-menu">
                <li class="nav-item"> <a class="nav-link" href="/user/video"> New Video </a></li>
                <li class="nav-item"> <a class="nav-link" href="/user/videoList"> List of Video </a></li>
              </ul>
            </div>
          </li>
          <li class="nav-item menu-items">
            ...
            <div class="content-wrapper">
                <div class="row">
                    <div class="col-12 grid-margin stretch-card">
                        <div class="card">
                            <h6 class="card-header"> Edit Vending Video
                                <a href="user/videoList" class="btn btn-danger float-end">Back</a>
                            </h6>
                            <div class="card-body">
                                @if (session('status'))
                                ...
                                @endif
                                <form class="forms-sample" method="POST" action="{{ url('update-video/'.$videos->id) }}" enctype="multipart/form-data">
                                    {{ csrf_field() }}
                                    @method('PUT')
                                    <div class="form-group">
                                        <label for="videoName">Video Name</label>
                                        <input type="text" name="video_name" value="{{$videos->video_name}}" class="form-control" id="videoName" placeholder="Video Name">
                                    </div>
                                    <div class="form-group">
                                      <label for="video">Video</label>
                                      <input type="file" name="video" value="{{$videos->video}}" class="form-control" id="video" placeholder="Video">
                                    </div>
                                    <div class="form-group">
                                      <label for="vendingID">Vending ID</label>
                                      <select name="vending_id" class="form-control" style="width:100%">
                                        @foreach ($videos as $data)
                                          <option value="{{$data->video_id}}">{{$data->video_id}}</option>
                                        @endforeach
                                      </select>
                                    </div>
                                    <button type="submit" class="btn btn-primary me-2">Update</button>
                                    <a class="btn btn-dark" href="user/videoList">Cancel</a>
                                </form>
                            </div>
                        </div>
                    </div>
