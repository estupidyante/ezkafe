@section('title')
<title>EzKafe | FAQs Categories</title>
@endsection
@section('page-style')
<style type="text/css">
    .category {
                                ...
                                @foreach ($categories as $row)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <form class="d-inline" action="{{ route('user.faqscategory.destroy', $row ) }}" method="POST">
                                                @method('DELETE')
                                                @csrf
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form method="POST" action="{{ route('user.faqscategory.create') }}">
            @csrf
...
<div class="modal fade" id="updateCategoryModal_{{ $category->id }}" tabindex="-1" aria-labelledby="updateCategoryModalLabel_{{ $category->id }}" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <p class="text-muted">Please answer all the input fields to add a new category.</p>
        <form method="POST" action="/user/faqscategory/{{ $category->id }}">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label>Name</label>
